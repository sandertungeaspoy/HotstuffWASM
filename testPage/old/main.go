...
func main() {
	...
	connections.connections = make(map[string]*websocket.Conn)
	flag.Parse()
	...
	go func() {
		for {
			// sigsExit := make(chan os.Signal, 1)
			// signal.Notify(sigsExit, os.Interrupt)
			// fmt.Println("Select / chan blocking")
			// select {
			// case <-sigsExit:
			// 	break
			// }
			...
			if _, ok := connections.connections["127.0.0.1:13721"]; ok {
				...
				if err != nil {
					...
					if _, ok := connections.connections["127.0.0.1:13711"]; ok {
						...
						writer.Close()
						cancel()
						// fmt.Fprintf(connections.connections["127.0.0.1:13722"], msg)
					}
					connections.mux.Unlock()
				}
			} else {
	...
	go func() {
		for {
			...
			connections.mux.Lock()
			if _, ok := connections.connections["127.0.0.1:13731"]; ok {
				conn := connections.connections["127.0.0.1:13731"]
				// conn.SetReadDeadline(time.Now().Add(time.Millisecond * 2000))
				connections.mux.Unlock()
				ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
				_, msg, err := conn.Read(ctx)
...
func (s wasmServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	connections.connections[dialing] = c
